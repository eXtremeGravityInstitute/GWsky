...
int init_useful_powers(UsefulPowers *p, double number)
{
    ...
    if(number < 0.0) number = 1.0e-10;  // To stop premature exit of code
	//CHECK(number >= 0.0, PD_EDOM, "number must be non-negative");
...
double AmpInsAnsatz(double Mf, UsefulPowers * powers_of_Mf, AmpInsPrefactors * prefactors) {
  ...
  return 1.0 + powers_of_Mf->two_thirds * prefactors->two_thirds

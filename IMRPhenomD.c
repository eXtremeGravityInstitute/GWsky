...
int IMRPhenomDGenerateh22FDAmpPhase(
  ...
  char errstr[200];
    ...
    if (fRef_in < 0.0) printf("fRef_in must be positive (or 0 for 'ignore')\n");
    if (m1 <= 0.0) printf("m1 must be positive\n");
    if (m2 <= 0.0) printf("m2 must be positive\n");
    if (f_min <= 0.0) printf("f_min must be positive\n");
    if (f_max < 0.0) printf("f_max must be greater than 0\n");
    if (distance <= 0.0) printf("distance must be positive\n");
  ...
  int status = IMRPhenomDGenerateh22FDAmpPhase_internal(h22, freq, findT, phi0, fRef, m1, m2, chi1, chi2, distance);
